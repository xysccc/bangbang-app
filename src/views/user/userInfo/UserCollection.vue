<template>
  <div class="myCollection_wrapped">
    <status-bar/>
    <top-bar :topBarLab="topLab"/>
    <div class="topLab">
      <div class="svg" ref="animation1"></div>
      <div class="fontLab">我的收藏</div>
    </div>
    <div class="collectionList" v-if="oneInfo.length">
      <div class="top_wrapped" v-for="(item,index) in oneInfo" :key="item.id" @click="router.push(`/allBang/${item.id}`)">
        <div class="top">
   ...
   <empty-cps v-if="!oneInfo.length"></empty-cps>
  </div>
...
import { api_Collection, api_form_qxsc, api_form_sc } from "@/request/api";
import { getTime, twoTimeInterval } from "@/utils/time";
import lottie from "lottie-web";
import add from "@/assets/json/clickJson.json";
import collection from "@/assets/json/collection.json"
import router from "@/router";
mui.back=function () {
  router.back()
}
const topLab=ref("我的收藏")
const time = computed(() => {
...
let oneInfo=ref({})
const getOne=async ()=>{
  const res:any=await api_Collection()
  if (res.result) {
...
onMounted(()=>{
  lottie.loadAnimation({
    ...
    animationData: collection,//渲染动效json
  })
})
</script>
...
@font-face {
  font-family: kkht;
  src: url("@/assets/fonts/TsangerYuYangT_W03_W03.ttf");
}
.icon {
  width: 1em;
  height: 1em;
