<template>
<div class="today_wrapped">
  <status-bar/>
  <top-bar :top-bar-lab="topLab"/>
  <div class="topLab">
    <div class="svg" ref="animation1"></div>
    <div class="fontLab">今日新单</div>
  </div>
  <div class="todayList" v-if="dataList.length">
...
import { api_form_qxsc, api_form_sc, api_todayBang } from "@/request/api";
import { Toast } from "vant";
import router from "@/router";
import { getTime, twoTimeInterval } from "@/utils/time";
import lottie from "lottie-web";
import newVal from "@/assets/json/new.json";
const topLab=ref('今日新单')
const dataList=ref([])
const getData=async ()=>{
  const res:any= await api_todayBang()
  console.log(res)
  if (res.code!==1) {
    Toast.fail('获取数据失败')
    router.back()
